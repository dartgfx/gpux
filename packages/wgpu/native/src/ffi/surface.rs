...
#[cfg(target_os = "android")]
#[no_mangle]
pub extern "system" fn Java_com_example_flutter_1wgpu_FlutterWgpuPlugin_nativeGetWindow(
    env: jni::JNIEnv,
...
#[cfg(target_os = "android")]
#[no_mangle]
pub extern "system" fn Java_com_example_flutter_1wgpu_FlutterWgpuPlugin_nativeReleaseWindow(
    _env: jni::JNIEnv,
    _class: jni::objects::JClass,
    ...
    if window_ptr != 0 {
        unsafe {
            use ndk::native_window::NativeWindow;
            use std::ptr::NonNull;
            let ptr = NonNull::new(window_ptr as *mut ndk::native_window::NativeWindow);
            if let Some(ptr) = ptr {
...
pub extern "C" fn wgpu_surface_present(surface_id: u64) {
    ...
    ffi_catch!((), {
        ...
        {
            let surface = unsafe { deref_handle_mut::<WgpuSurface>(surface_id) };
            crate::surface::windows::copy_wgpu_to_pixel_buffer(surface);
        }

        #[cfg(target_os = "linux")]
        {
            let surface = unsafe { deref_handle_mut::<WgpuSurface>(surface_id) };
            crate::surface::linux::copy_wgpu_to_pixel_buffer(surface);
        }
